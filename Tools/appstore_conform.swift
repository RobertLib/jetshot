...
//          [<music.m4a> [gain]]
//
//  The bed is one of the game's own tracks out of jetshot/Music/ — see PREVIEW_MUSIC
//  in Tools/appstore_media.sh.
//
...
//  find out. The native device resolution is *not* accepted — every iPhone slot,
//  6.5" included, takes 886 × 1920 and nothing else — and Level 4.0 is a real
//  ceiling, so a preview at the device's own size comes out as Level 5.0 and is
//  refused. Silence does not satisfy the audio requirement either: AAC compresses
//  digital silence to about 2 kbps, two orders of magnitude under the 256 kbps
//  asked for, and a preview with no usable audio is treated as an unsupported
//  audio configuration. Hence the music bed — pass a file and it is looped under
//  the whole cut.
//
...
// a target — the encoder spends what the picture needs and no more. Jetshot is
// mostly black: a starfield and a few dozen bright vector shapes over an empty
// sky, which inter-frame prediction eats for almost nothing between explosions.
// Asking for 14 Mbps with an ordinary one-second GOP is how a preview ends up
// under the 10 Mbps floor and is refused for a reason the error will not name.
//
...
// lands on the number asked for — 11.5 Mbps requested is 11.5 Mbps delivered,
// which sits in the middle of Apple's band, well inside Level 4.0's ceiling and
// nowhere near Connect's 500 MB. Do not "optimise" the GOP back without
// re-measuring the finished file: this script prints the rate it actually wrote,
// and that figure is the whole check.
...
func transfer(from output: AVAssetReaderOutput, to input: AVAssetWriterInput,
    ...
    let queue = DispatchQueue(label: "cz.rob.jetshot.appstore-conform")
