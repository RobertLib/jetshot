...
class MenuScene: SKScene {
    ...
    private func setupGameCenterButton() {
        gameCenterButton = UITheme.createButton(
            text: "GAME CENTER",
            color: UITheme.Colors.accentGreen,
            width: UITheme.Dimensions.buttonWidthLarge,
            name: "gameCenterButton"
        )
        gameCenterButton.position = CGPoint(x: size.width / 2, y: size.height / 2 - 137.5)

        // Add glow effect
        GlowHelper.addEnhancedGlow(to: gameCenterButton, color: UITheme.Colors.accentGreen, intensity: 0.2)
