...
class LevelSelectScene: SKScene {
    ...
    private func createLevelButton(level: Int, size: CGFloat) -> SKNode {
        ...
        if isCompleted {
            ...
            // Display score for completed level
            if let levelScore = levelManager.getLevelScore(level: level) {
                ...
                scoreLabel.fontColor = UITheme.Colors.primaryGoldLight
                scoreLabel.horizontalAlignmentMode = .center
                scoreLabel.verticalAlignmentMode = .center
                scoreLabel.position = CGPoint(x: 0, y: -size / 2 - 14)
