...
class EnemyManager {
    ...
    func update(currentTime: TimeInterval) {
        ...
        if currentWave.isFormation && enemiesSpawnedInWave == 0 {
            ...
            formationManager?.spawnFormation(
                ...
                attackDelay: 2.0,
                onEnemyComplete: { [weak self] in
                    // Enemy left the screen
                    self?.onEnemyLeftScreen?()
                }
            )
    ...
    private func spawnEnemy(type: EnemyType) {
        ...
        enemy.startMovement { [weak self] in
            // Enemy left the screen
            self?.onEnemyLeftScreen?()
        }
    }
    ...
    func pauseSpawning() {
        isPaused = true
        formationManager?.pauseFormations()
    }

    func resumeSpawning() {
        isPaused = false
        formationManager?.resumeFormations()
    }
