...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func showCountdown(in parentNode: SKNode, background: SKSpriteNode, completion: @escaping () -> Void) {
        ...
        let numbers = ["3", "2", "1", "START!"]

        for (index, number) in numbers.enumerated() {
            let showNumber = SKAction.run {
            ...
            let scaleUp = SKAction.scale(to: index == 3 ? 1.1 : 1.0, duration: 0.15)
            let appear = SKAction.group([fadeIn, scaleUp])

            let wait = SKAction.wait(forDuration: index == 3 ? 0.5 : 0.6)

            let fadeOut = SKAction.fadeOut(withDuration: 0.15)

            countdownActions.append(showNumber)
            countdownActions.append(appear)
            countdownActions.append(wait)
            countdownActions.append(fadeOut)
        }
