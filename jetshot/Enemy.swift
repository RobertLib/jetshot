...
class Enemy: SKShapeNode {
    ...
    private func setupEnemy(sceneSize: CGSize) {
        ...
        // Add 3D vector effects for specific enemy types
        add3DVectorEffects()
    }

    /// Adds pseudo-3D vector effects based on enemy type
    private func add3DVectorEffects() {
        let baseSize: CGFloat = 12
        let sizeMultiplier = enemyType.size
        let effectRadius = baseSize * sizeMultiplier * 1.5

        switch enemyType {
        case .sniper:
            // Add scanning cone that appears when targeting
            let scanCone = VectorFX3D.createScanCone(
                length: 150,
                angle: .pi / 8,
                color: UIColor.red.withAlphaComponent(0.6),
                direction: CGPoint(x: 0, y: -1)
            )
            scanCone.name = "scanCone"
            scanCone.alpha = 0
            scanCone.zPosition = -2
            addChild(scanCone)

            // Add targeting radar
            let radar = VectorFX3D.createRadarSweep(
                radius: effectRadius * 1.2,
        ...
        case .vortex:
            // Add rotating vector aura
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius,
                color: UIColor(red: 0.5, green: 0.2, blue: 0.8, alpha: 1.0),
                style: .octagon
            )
            aura.zPosition = -1
            addChild(aura)
            ...
            let forceField = VectorFX3D.createForceField(
                radius: effectRadius * 1.3,
                color: UIColor(red: 0.5, green: 0.2, blue: 0.8, alpha: 0.5)
            )
            forceField.zPosition = -2
            addChild(forceField)

            // Add gravitational waves showing pull radius (150 = gravity radius from GameScene)
            for i in 1...3 {
                let delay = TimeInterval(i) * 0.4
                let gravityWave = SKShapeNode(circleOfRadius: 5)
                gravityWave.strokeColor = UIColor(red: 0.7, green: 0.4, blue: 1.0, alpha: 0.8)
                gravityWave.fillColor = .clear
                gravityWave.lineWidth = 2.0
                gravityWave.zPosition = -3
                addChild(gravityWave)

                // Pulsating wave animation
                let expandScale = SKAction.scale(to: 150 / 5, duration: 1.2) // Scale to gravity radius
                let fadeOut = SKAction.fadeOut(withDuration: 1.2)
                let reset = SKAction.group([
                    SKAction.scale(to: 1.0, duration: 0.01),
                    SKAction.fadeAlpha(to: 0.8, duration: 0.01)
                ])
                let pulse = SKAction.sequence([
                    SKAction.wait(forDuration: delay),
                    SKAction.group([expandScale, fadeOut]),
                    reset
                ])
                gravityWave.run(SKAction.repeatForever(pulse))
            }

        case .shield:
            // Add protective shield sphere
            let shieldSphere = VectorFX3D.createShieldSphere(
                radius: effectRadius * 1.2,
                color: UIColor(red: 0.2, green: 0.8, blue: 0.3, alpha: 1.0),
                persistent: true
            )
            shieldSphere.name = "enemyShieldSphere"
            shieldSphere.zPosition = -1
            addChild(shieldSphere)

            // Add force field
            let forceField = VectorFX3D.createForceField(
                radius: effectRadius * 1.4,
                color: UIColor(red: 0.2, green: 0.8, blue: 0.3, alpha: 0.4)
            )
            forceField.zPosition = -2
            addChild(forceField)

        case .ghost:
            // Add ethereal hexagon aura
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius,
                color: UIColor(red: 0.7, green: 0.7, blue: 0.9, alpha: 0.8),
                style: .hexagon
            )
            aura.zPosition = -1
            aura.alpha = 0.6
        ...
        case .laser:
            ...
            // Add beam charger effect
            let beamCharger = VectorFX3D.createBeamCharger(
                length: effectRadius * 2,
                color: UIColor.red
            )
            beamCharger.name = "beamCharger"
            beamCharger.zPosition = -2
            beamCharger.alpha = 0
            addChild(beamCharger)

        case .commander, .eliteGuard:
            // Add prestigious hexagon aura
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius,
            ...
            // Add energy core for commanders
            if enemyType == .commander {
                let energyCore = VectorFX3D.createEnergyCore(
                    radius: effectRadius * 0.8,
                    color: enemyType.strokeColor,
                    layers: 3
                )
        ...
        case .spinner:
            ...
            // Add additional spinning circles
            for i in 1...2 {
                let extraAura = VectorFX3D.createVectorAura(
                    radius: effectRadius * (1.0 + CGFloat(i) * 0.3),
                    color: UIColor.white.withAlphaComponent(0.5),
                    style: .circle
                )
                extraAura.zPosition = -2
                addChild(extraAura)
            }

        case .teleporter:
            // Add circular teleport aura
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius,
                color: UIColor.cyan,
                style: .circle
        ...
        case .tank:
            // Add orbital defense system
            let orbitalDefense = VectorFX3D.createOrbitalDefense(
                count: 4,
                radius: effectRadius * 1.2,
                color: enemyType.strokeColor
            )
            orbitalDefense.zPosition = -1
            addChild(orbitalDefense)

            // Add force field
            let forceField = VectorFX3D.createForceField(
                radius: effectRadius * 1.4,
                color: enemyType.color.withAlphaComponent(0.4)
            )
            forceField.zPosition = -2
            addChild(forceField)

        case .heavy:
            // Add rotating cannons
            let cannons = VectorFX3D.createRotatingCannons(
                count: 4,
                radius: effectRadius * 1.1,
                color: enemyType.strokeColor
            )
            cannons.zPosition = -1
            addChild(cannons)

        case .turret, .turretSpiral:
            // Add radar sweep for targeting
            let radar = VectorFX3D.createRadarSweep(
                radius: effectRadius * 1.3,
                color: enemyType.color.withAlphaComponent(0.5)
            )
            radar.zPosition = -2
            addChild(radar)

            // Add rotating cannons
            let cannonCount = enemyType == .turret ? 4 : 1
            let cannons = VectorFX3D.createRotatingCannons(
                count: cannonCount,
        ...
        case .mine:
            // Add radar warning
            let radar = VectorFX3D.createRadarSweep(
                radius: effectRadius * 1.5,
                color: UIColor.red.withAlphaComponent(0.5)
            )
            radar.name = "mineRadar"
            radar.zPosition = -1
            radar.alpha = 0.3
            addChild(radar)

        case .scout:
            // Add simple circle aura
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius * 0.8,
                color: enemyType.color,
                style: .circle
        ...
        case .splitter:
            // Add triangle aura (shows it will split)
            let aura = VectorFX3D.createVectorAura(
                radius: effectRadius,
                color: enemyType.color,
                style: .triangle
    ...
    func startMovement(completion: @escaping () -> Void) {
        ...
        case .shield:
            // Shield: moves down with rotating shield (shield already rotates from setupEnemy)
            performShieldMovement(completion: completion)

        case .splitter:
            // Splitter: moves down normally (splits on death)
            performSplitterMovement(completion: completion)

        case .laser:
            // Laser: moves down while charging beam attacks
            performLaserMovement(completion: completion)
        ...
        case .vortex:
            // Vortex: moves down while creating vortex effect
            performVortexMovement(completion: completion)
        ...
        case .mirror:
            // Mirror: moves down with pulsating effect
            performMirrorMovement(completion: completion)
    ...
    private func startTurretSpiralDockMode() {
        ...
        // Play turret dock sound
        if let gameScene = gameScene {
            SoundManager.shared.playTurretDockSound(on: gameScene)
        }

        // Start rotating the turret continuously - use negative angle to rotate clockwise through lower half
    ...
    private func armMine() {
        ...
        // Play mine arming sound
        if let gameScene = gameScene {
            SoundManager.shared.playMineArmSound(on: gameScene)
        }
    ...
    private func startMineCountdown(completion: @escaping () -> Void) {
        ...
        // Activate mine radar warning - make it pulse faster and more visible
        if let radar = childNode(withName: "mineRadar") {
            radar.run(SKAction.sequence([
                SKAction.fadeAlpha(to: 0.9, duration: 0.2),
                SKAction.run {
                    // Speed up radar rotation during countdown
                    radar.removeAllActions()
                    if let sweepLine = radar.children.first(where: { $0 is SKShapeNode && $0.name != "radarCircle" }) {
                        let fastRotate = SKAction.rotate(byAngle: .pi * 2, duration: 0.5) // Much faster
                        sweepLine.run(SKAction.repeatForever(fastRotate))
                    }
                }
            ]))
        }
        ...
        let updateAction = SKAction.run { [weak self] in
            ...
            if let gameScene = self.gameScene {
                SoundManager.shared.playMineCountdownSound(on: gameScene)
            }
    ...
    private func shootAimed(scene: SKScene) {
        ...
        // Show scan cone when targeting (if not already visible)
        if let scanCone = childNode(withName: "scanCone") {
            if scanCone.alpha < 0.5 {
                scanCone.run(SKAction.fadeAlpha(to: 0.7, duration: 0.2))
            }
        }

        // Create targeting reticle on player
        let lockDuration: TimeInterval = 0.8
        let reticle = VectorFX3D.createTargetingReticle(
        ...
        if let gameScene = scene as? GameScene {
            gameScene.gameContentNode.addChild(reticle)
        } else {
            scene.addChild(reticle)
        }

        // Wait for lock, then shoot
        run(SKAction.sequence([
            SKAction.wait(forDuration: lockDuration),
            SKAction.run { [weak self] in
                self?.executeAimedShot(scene: scene, playerPosition: playerNode.position)

                // Fade out scan cone after shooting
                if let scanCone = self?.childNode(withName: "scanCone") {
                    scanCone.run(SKAction.fadeAlpha(to: 0.2, duration: 0.5))
                }
            }
        ]))
    }

    private func executeAimedShot(scene: SKScene, playerPosition: CGPoint) {
        // Play enemy shoot sound
        if let gameScene = gameScene {
            SoundManager.shared.playEnemyShootSound(on: gameScene)
        }
        ...
        let moveAction = SKAction.move(to: playerPosition, duration: bulletDuration)
        let removeAction = SKAction.removeFromParent()
        bullet.run(SKAction.sequence([moveAction, removeAction]))
    }
    ...
    private func performBouncerMovement(completion: @escaping () -> Void) {
        ...
        let updateAction = SKAction.run { [weak self] in
            ...
            if newX < 30 || newX > self.sceneSize.width - 30 {
                ...
                // Play bounce sound
                if let gameScene = self.gameScene {
                    SoundManager.shared.playBouncerBounceSound(on: gameScene)
                }
            }
    ...
    private func performTeleporterMovement(completion: @escaping () -> Void) {
        ...
        let teleportAction = SKAction.run { [weak self] in
            ...
            // Play teleport sound
            if let gameScene = self.gameScene {
                SoundManager.shared.playTeleportSound(on: gameScene)
            }
    ...
    private func shootLaser(scene: SKScene) {
        // Laser enemy charges up and fires a continuous beam
        ...
        // Play charging sound
        if let gameScene = gameScene {
            SoundManager.shared.playLaserChargeSound(on: gameScene)
        }

        // Create charging effect
        let chargeIndicator = SKShapeNode(circleOfRadius: 8)
        chargeIndicator.fillColor = .clear
        chargeIndicator.strokeColor = UIColor(red: 1.0, green: 0.2, blue: 0.2, alpha: 1.0)
        chargeIndicator.lineWidth = 3
        chargeIndicator.position = CGPoint(x: 0, y: -15)
        chargeIndicator.name = "chargeIndicator"
        addChild(chargeIndicator)

        // Activate beam charger visual if it exists
        if let beamCharger = childNode(withName: "beamCharger") {
            beamCharger.run(SKAction.fadeAlpha(to: 0.8, duration: 0.2))
        }

        // Add charge-up particle effect
        let chargeEffect = VectorFX3D.createChargeEffect(
            at: CGPoint(x: position.x, y: position.y - 15),
            color: UIColor.red,
            duration: 1.0
        )
        if let gameScene = scene as? GameScene {
            gameScene.gameContentNode.addChild(chargeEffect)
        } else {
            scene.addChild(chargeEffect)
        }
        ...
        chargeIndicator.run(SKAction.repeat(pulse, count: 3))

        // After charging, fire laser beam
        let chargeTime = 1.0
        let wait = SKAction.wait(forDuration: chargeTime)
        let fireLaser = SKAction.run { [weak self] in
            guard let self = self else { return }
            self.fireLaserBeam(scene: scene)
            chargeIndicator.removeFromParent()

            // Deactivate beam charger
            if let beamCharger = self.childNode(withName: "beamCharger") {
        ...
        run(SKAction.sequence([wait, fireLaser]))
    }

    private func fireLaserBeam(scene: SKScene) {
        // Play laser fire sound
        if let gameScene = gameScene {
            SoundManager.shared.playLaserFireSound(on: gameScene)
        }

        // Create laser beam from enemy to bottom of screen
        let beamWidth: CGFloat = 8
        let beamLength = position.y + 50

        let beam = SKShapeNode(rectOf: CGSize(width: beamWidth, height: beamLength))
        beam.fillColor = UIColor(red: 1.0, green: 0.2, blue: 0.2, alpha: 0.8)
        beam.strokeColor = UIColor(red: 1.0, green: 0.5, blue: 0.5, alpha: 1.0)
        beam.lineWidth = 2
        beam.position = CGPoint(x: position.x, y: position.y - beamLength / 2)
        beam.name = "laserBeam"
        beam.zPosition = -1

        // Add glow to laser beam
        GlowHelper.addEnhancedGlow(to: beam, color: UIColor(red: 1.0, green: 0.2, blue: 0.2, alpha: 1.0), intensity: 1.5)

        // Setup physics for laser beam
        beam.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: beamWidth, height: beamLength))
        beam.physicsBody?.isDynamic = false
        beam.physicsBody?.categoryBitMask = PhysicsCategory.enemyBullet
        beam.physicsBody?.contactTestBitMask = PhysicsCategory.player
        beam.physicsBody?.collisionBitMask = PhysicsCategory.none

        // Add beam to scene
        if let gameScene = scene as? GameScene {
            gameScene.gameContentNode.addChild(beam)
        } else {
            scene.addChild(beam)
        }
    ...
    // MARK: - New Enemy Movement Patterns

    private func performShieldMovement(completion: @escaping () -> Void) {
        // Shield enemy: moves down with periodic side-to-side wobble for defensive positioning
        let startX = position.x
        let startY = position.y
        let endY: CGFloat = -20
        let totalDistance = startY - endY
        ...
        for i in 0...steps {
            let progress = CGFloat(i) / CGFloat(steps)
            let y = startY - (totalDistance * progress)
            // Gentle side-to-side wobble (defensive stance)
            let x = startX + sin(progress * .pi * 3) * 25
            let clampedX = max(30, min(sceneSize.width - 30, x))
            pathPoints.append(CGPoint(x: clampedX, y: y))
        }
    ...
    private func performSplitterMovement(completion: @escaping () -> Void) {
        // Splitter: wavy movement that suggests it will split
        let startX = position.x
        let startY = position.y
        let endY: CGFloat = -20
        let totalDistance = startY - endY
        ...
        for i in 0...steps {
            ...
            let x = startX + sin(progress * .pi * 4) * 40
            let clampedX = max(30, min(sceneSize.width - 30, x))
            pathPoints.append(CGPoint(x: clampedX, y: y))
        }
        ...
        // Add pulsating effect to hint at splitting
        let pulse = SKAction.sequence([
            SKAction.scale(to: 1.1, duration: 0.5),
            SKAction.scale(to: 1.0, duration: 0.5)
        ])
        run(SKAction.repeatForever(pulse), withKey: "splitterPulse")
    }

    private func performLaserMovement(completion: @escaping () -> Void) {
        // Laser enemy: descends slowly, stops periodically to charge and fire
        let startY = position.y
        let endY: CGFloat = -20
        let totalDistance = startY - endY
        let segments = 3 // Number of stops to fire laser

        var actions: [SKAction] = []

        for i in 0..<segments {
            let segmentDistance = totalDistance / CGFloat(segments + 1)
            let targetY = startY - segmentDistance * CGFloat(i + 1)
            let moveTime = movementDuration / TimeInterval(segments * 3)

            // Move to position
            actions.append(SKAction.moveTo(y: targetY, duration: moveTime))

            // Stop and charge laser
            actions.append(SKAction.wait(forDuration: 0.5))

            // Trigger laser shot (handled by existing shootLaser method)
            if let scene = scene {
                actions.append(SKAction.run { [weak self] in
                    self?.shootLaser(scene: scene)
                })
            }
        ...
        // Final descent
        let finalMoveTime = movementDuration / TimeInterval(segments * 2)
        actions.append(SKAction.moveTo(y: endY, duration: finalMoveTime))
        ...
        // Start shooting behavior for other times
        startShooting()
    }

    private func performVortexMovement(completion: @escaping () -> Void) {
        // Vortex: slow spiral descent creating a vortex effect
        let startX = position.x
        let startY = position.y
        let endY: CGFloat = -20
        let totalDistance = startY - endY
        ...
        for i in 0...steps {
            ...
            let radius = progress * 60
            let angle = progress * .pi * 4
        ...
        // Continuous rotation for vortex effect
        let rotate = SKAction.rotate(byAngle: .pi * 2, duration: 2.0)
        run(SKAction.repeatForever(rotate), withKey: "vortexRotation")
        ...
        run(SKAction.repeatForever(pulse), withKey: "vortexPulse")
    }

    private func performMirrorMovement(completion: @escaping () -> Void) {
        // Mirror: smooth sinusoidal movement with shimmer effect
        let startX = position.x
        let startY = position.y
        let endY: CGFloat = -20
        let totalDistance = startY - endY
        ...
        for i in 0...steps {
            ...
            let y = startY - (totalDistance * progress)
            // Smooth sine wave
            let x = startX + sin(progress * .pi * 2.5) * 55
            let clampedX = max(30, min(sceneSize.width - 30, x))
            pathPoints.append(CGPoint(x: clampedX, y: y))
        }
        ...
        // Shimmer effect - fade between normal and bright
        let shimmer = SKAction.sequence([
            SKAction.fadeAlpha(to: 0.6, duration: 0.7),
            SKAction.fadeAlpha(to: 1.0, duration: 0.7)
        ])
        run(SKAction.repeatForever(shimmer), withKey: "mirrorShimmer")
        ...
        run(SKAction.repeatForever(wobble), withKey: "mirrorWobble")
    }
}
