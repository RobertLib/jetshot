...
class Enemy: SKShapeNode {
    ...
    init(sceneSize: CGSize, scene: SKScene, type: EnemyType = .basic) {
        ...
        // Don't start shooting immediately for kamikaze, turret, turretSpiral, mine and meteorSwarm
        if type != .kamikaze && type != .turret && type != .turretSpiral && type != .mine && type != .meteorSwarm {
            startShooting()
        }
    }
    ...
    private func setupEnemy(sceneSize: CGSize) {
        ...
        // Add cockpit/core detail (skip for meteor swarm and flanker)
        if enemyType != .meteorSwarm && enemyType != .flanker {
