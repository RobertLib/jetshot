...
class GlowHelper {
    ...
    /// Adds an enhanced multi-layer neon glow effect
    /// - Parameters:
    ///   - node: The shape node to add glow to
    ///   - color: The color of the glow (defaults to the node's fill color)
    ...
    static func addEnhancedGlow(to node: SKShapeNode, color: UIColor? = nil, intensity: CGFloat = 1.0) {
        ...
        // Multi-layer approach for smooth, balanced glow
        // Inner bright core layers
        for i in 1...2 {
            let glowNode = SKShapeNode()
            glowNode.path = node.path
            glowNode.fillColor = glowColor
            ...
            let scale = 1.0 + (CGFloat(i) * 0.08)
            let alpha = (0.5 - CGFloat(i) * 0.1) * intensity
        ...
        for i in 3...5 {
            ...
            let alpha = (0.3 / CGFloat(i - 1)) * intensity
